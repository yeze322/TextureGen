...
def get_bounds(images, im_size):
    ...
    ele = (lowerbound,upperbound)
    bounds = [ ele for b in range(im_size[0]*im_size[1] * images[0].shape[1])]
    return bounds 
...
def gram_matrix(activations):
    ...
    '''
    #e.g. actv.shape = (1,3,20,20)
    N = activations.shape[1] #N = 3 -> channels
    F = activations.reshape(N,-1) #F.shape = 3*400
    M = F.shape[1] #M = 400 (pixels number in one dimension)
    G = np.dot(F,F.T) / M #G: feature matrix in the paper
    return G
...
#sub-functions
def calcImgSize(im_size , imgShape):
    if isinstance(im_size,float):
        # width/height
        im_scale = np.sqrt(im_size **2 /np.prod(np.asarray(imgShape[:2])))
        im_size = im_scale * np.asarray(imgShape[:2])
    return im_size

def modifyNetInputSize(net_model, batchsize, im_size):
    data = getlines(net_model)
    data[2] = "input_dim: %i\n" %(batchsize)
    data[4] = "input_dim: %i\n" %(im_size[0])
    data[5] = "input_dim: %i\n" %(im_size[1])
...
def load_net(net_model, net_weights, net_mean):
...
def load_image(im_name, show_img):
...
def load_resources(img, im_size, net_model, net_weights, mean, show_img=False):
    '''
    Loads and preprocesses image into caffe format by constructing and using the appropriate network.

    :param im_name: file name of the image to be loaded
    ...
    '''
    im_size = calcImgSize(im_size, img.shape)
    modifyNetInputSize(net_model=net_model, batchsize=1, im_size=im_size)
    print "Image loaded!"
    net_mean =  np.tile(mean[:,None,None],(1,) + im_size-1)#3*0 -> 3*200*200
    net = load_net(net_model, net_weights, net_mean)
    print "Net loaded!"
    #load pretrained network
    img_pp = net.transformer.preprocess('data',img)[None,:]
    if show_img:
    	print img_pp.shape
    	plt.imshow(img_pp[0,0,:,:], cmap='gray')
    	plt.show()
    return img_pp, net